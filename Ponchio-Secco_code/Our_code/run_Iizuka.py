...
model = data.model
# Switch the model to evaluation mode, so disables dropout, fixes batchnorm behavior etc
model.evaluate()
...
for img_name in image_list:
    ...
    for mask_name in mask_list:
        ...
        # Invert mask (model convention): the masked region needs to be in white but we
        # generated the masks the opposite way.
        M = 1.0 - M
        ...
        # Postprocess with Poisson blending (optional)
        if opt.postproc:
