...
# Random walk where length is the number of steps to take in the random walk (therefore it is an upper limit on the
# number of masked pixels, since backtracking is allowed)
...
# Random walk where target_ratio is exactly the ratio of pixels to be corrupted
...
# Salt and pepper noise with random sampling according to the given ratio of masked pixels
...
# Box-shaped masks: the passed initial position is randomly considered one of the vertices
...
# Circular masks: the size is controlled by the radius and starting point is considered
...
if __name__ == '__main__':
	...
	parser.add_argument('--mode', type=str, default="all", 
			help = "Mask type:\n box - for rectangular boxes || sap - for salt and pepper noise || circle - for circular masks || all - for random walk masks with the default ratios of noise || rw - for random walk masks with user defined ratio of noise ||  rw_ratio - for random walks that will always reach the passed ratio")
